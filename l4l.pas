...
{ Levels ----------------------------------------------------------------------}
  ...
  private
    ...
    FName: string;
  protected
    constructor Create(aName: string; Level: Integer);
  public
    ...
    class function GetLevel(aName: string): TLogLevel; overload;
    { Retrieve a level object given its name, or default if not valid. }
    class function GetLevel(aName: string; DefaultLevel: TLogLevel): TLogLevel;
      overload;
  end;
...
{ Loggers ---------------------------------------------------------------------}
  ...
  { This is the central class in the l4l package. One of the distinctive
    features of l4l is hierarchical loggers and their evaluation. }
  ...
  public
    ...
    procedure RemoveAppender(const aName: string); overload;
  ...
  private
    procedure SetLevel(const aLevel: TLogLevel);
  public
...
{ Layouts ---------------------------------------------------------------------}
  ...
  { Abstract base for layouts.
    ...
    l4l.appender.<name>.layout.dateFormat=yyyy/mm/dd hh:nn:ss.zzz
  }
  ...
  { This layout outputs events in a HTML table.
    ...
    l4l.appender.<name>.layout.title=Logging Messages
  }
  ...
  { Layout based on specified pattern.
    ...
    l4l.appender.<name>.layout.pattern=%r [%t] %p %c %x - %m%n
  }
...
{ ErrorHandler ----------------------------------------------------------------}
  ...
  protected
    procedure SetLogger(const aLogger: TLogLogger); override;
  public
...
{ Filters ---------------------------------------------------------------------}
  ...
  { Abstract base class for filters - handles basic option setting.
    ...
    l4l.appender.<name>.filter1=TLogCustomFilter
    # Accept or reject the log event when deciding, Boolean, optional, default true
    l4l.appender.<name>.filter1.acceptOnMatch=true
  }
  ...
  protected
    ...
    procedure SetOption(const Name, Value: string); override;
  public
    constructor Create(const OnMatch: Boolean = True); reintroduce;
  ...
  { Filter by the message's level.
    ...
    Accepts the following options (as well as the standard OnMatch one):

    # Class identification
    l4l.appender.<name>.filter1=TLogLevelMatchFilter
    # Logging level to match on, Level, mandatory
    l4l.appender.<name>.filter1.match=warn
  }
  ...
  public
    constructor Create(const MatchLevel: TLogLevel;
      const OnMatch: Boolean = True); reintroduce;
    property AcceptOnMatch;
  ...
  { Filter by the message's level being within a range.
    ...
    # Class identification
    l4l.appender.<name>.filter1=TLogLevelRangeFilter
    # Minimum logging level to match on, Level, mandatory
    l4l.appender.<name>.filter1.minimum=warn
    # Maximum logging level to match on, Level, mandatory
    l4l.appender.<name>.filter1.maximum=error
  }
...
{ Appenders -------------------------------------------------------------------}
  ...
  protected
    procedure DoAppend(const Message: string); override;
  public
    constructor Create(const aName: string; const Stream: TStream;
      const aLayout: ILogLayout = nil); reintroduce; virtual;
    destructor Destroy; override;
  end;
  ...
  { Send log messages to a file.
    ...
    # Class identification
    l4l.appender.<name>=TLogFileAppender
    # Name of the file to write to, string, mandatory
    l4l.appender.<name>.fileName=C:\Logs\App.log
    # Whether to append to file, Boolean, optional, defaults to true
    l4l.appender.<name>.append=false
  }
  ...
  private
    ...
    FFileName: TFileName;
  protected
    procedure SetOption(const aName, Value: string); override;
    procedure SetLogFile(const aName: string);
    procedure CloseLogFile;
  public
    constructor Create(const aName, FileName: string;
      const aLayout: ILogLayout = nil; const Appnd: Boolean = True);
      reintroduce; virtual;
...
{ Configurators ---------------------------------------------------------------}
  ...
  { Extends BasicConfigurator to provide configuration from an external file.
    ...
    It is sometimes useful to see how l4l is reading configuration files.
    You can enable l4l internal logging by defining the l4l.debug variable. }
...
implementation
 ...
 resourcestring
   ...
   PleaseInitMsg           = 'Please initialise the l4l system properly';
 ...
 constructor TLogLevel.Create(aName: string; Level: Integer);
 begin
   inherited Create;
   FName  := Name;
   FLevel := Level;
 ...
 { Retrieve a level object given its name. }
 class function TLogLevel.GetLevel(aName: string): TLogLevel;
 begin
   Result := GetLevel(aName, Debug);
 end;

 { Retrieve a level object given its name, or default if not valid. }
 class function TLogLevel.GetLevel(aName: string; DefaultLevel: TLogLevel):
   TLogLevel;
 var
   Index: Integer;
 begin
   Result := DefaultLevel;
   for Index := 0 to Levels.Count - 1 do
     if TLogLevel(Levels[Index]).Name = aName then
     begin
       Result := TLogLevel(Levels[Index]);
       Exit;
     end;
 ...
 begin
   inherited Create;
   InitCriticalSection(FCriticalLogger);
   FAdditive  := True;
 ...
 function TLogLogger.GetAppender(const aName: string): ILogAppender;
 var
   Index: Integer;
 begin
   Result := nil;
   ...
   try
     for Index := 0 to FAppenders.Count - 1 do
       if ILogAppender(FAppenders[Index]).Name = aName then
       begin
 ...
 { Create new loggers via the logger class. }
 class function TLogLogger.GetLogger(const aName: string;
   const Factory: ILogLoggerFactory): TLogLogger;
 begin
   Result := DefaultHierarchy.GetLogger(aName, Factory);
 end;
 ...
 procedure TLogLogger.RemoveAppender(const aName: string);
 var
   Appender: ILogAppender;
 begin
   LockLogger;
   try
     Appender := GetAppender(aName);
 ...
 constructor TLogRoot.Create(const aLevel: TLogLevel);
 begin
 ...
 procedure TLogRoot.SetLevel(const aLevel: TLogLevel);
 begin
   if Level = nil then
   begin
     LogLog.Error(NilLevelMsg);
     inherited Level := l4l.Debug;
   end
   else
 ...
 procedure TLogLog.DoLog(const LogLevel: TLogLevel; const Message: string;
   const Err: Exception);
 begin
   if (LogLevel <> l4l.Debug) or InternalDebugging then
 ...
 begin
   inherited Create;
   InitCriticalSection(FCriticalHierarchy);
 ...
 begin
   ...
   begin
     ...
     begin
       ...
       FPatternParts.AddObject(Part, TObject(PartType));
       Part := '';
       Inc(Index);
     end
     else
     begin
       ...
       FPatternParts.AddObject(Part, TObject(ppText));
       Part := '';
     end;
   end;
 end;
 ...
 procedure TLogFallbackErrorHandler.SetLogger(const aLogger: TLogLogger);
 begin
   if FLoggers.IndexOf(aLogger) = -1 then
   begin
     FLoggers.Add(aLogger);
     LogLog.Debug(Format(AddingLoggerMsg, [aLogger.Name]));
   end;
 end;
 ...
 { Initialisation. }
 constructor TLogCustomFilter.Create(const OnMatch: Boolean);
 begin
   inherited Create;
   Self.AcceptOnMatch := OnMatch;
 end;
 ...
 constructor TLogLevelMatchFilter.Create(const MatchLevel: TLogLevel;
   const OnMatch: Boolean);
 begin
   inherited Create(OnMatch);
 ...
 constructor TLogStringFilter.Create(const Match: string;
   const IgnoreCase: Boolean; const OnMatch: Boolean);
 begin
   inherited Create(OnMatch);
   Self.Match      := Match;
...
{ Do nothing. }
 ...
 constructor TLogStreamAppender.Create(const aName: string; const Stream: TStream;
   const aLayout: ILogLayout);
 begin
 ...
 constructor TLogFileAppender.Create(const aName, FileName: string;
   const aLayout: ILogLayout; const Appnd: Boolean);
 begin
   inherited Create(aName, nil, aLayout);
   FAppend := Appnd;
   SetOption(FileNameOpt, FileName);
 end;

 { create file stream }
 procedure TLogFileAppender.SetLogFile(const aName: string);
 var
   strPath: string;
   f : TextFile;
 begin
 ...
 procedure TLogFileAppender.SetOption(const aName, Value: string);
 begin
   inherited SetOption(aName, Value);
   EnterCriticalSection(FCriticalAppender);
   try
 ...
 begin
   ...
   begin
     ...
     begin
       afilename := FFileName + '.' + IntToStr(i);
       if FileExists(afilename) then
         RenameFile(afilename, FFileName + '.' + IntToStr(i+1));
     end;
     // close file
     CloseLogFile;
 ...
 procedure TLogRollingFileAppender.SetOption(const aName, Value: string);
 var suffix : string;
 begin
   inherited SetOption(aName, Value);
   EnterCriticalSection(FCriticalAppender);
 ...
 type
   ...
   public
     Appender: ILogAppender;
     constructor Create(Appendr: ILogAppender);
   end;

 constructor TAppender.Create(Appendr: ILogAppender);
 begin
   inherited Create;
   Self.Appender := Appendr;
 end;
 ...
 { Read configuration from properties. The existing configuration is not
   ...
   l4l.appender.appenderName=nameOfAppenderClass

   # Set appender specific options.
   l4l.appender.appenderName.option1=value1
     :
   l4l.appender.appenderName.optionN=valueN

   For each named appender you can configure its ErrorHandler.
   The syntax for configuring an appender's error handler is:

   l4l.appender.appenderName.errorHandler=nameOfErrorHandlerClass

   For each named appender you can configure its Layout.
   The syntax for configuring an appender's layout and its options is:

   l4l.appender.appenderName.layout=nameOfLayoutClass
   l4l.appender.appenderName.layout.option1=value1
     :
   l4l.appender.appenderName.layout.optionN=valueN
   ...
   l4l.appender.appenderName.filterx=nameOfFilterClass
   l4l.appender.appenderName.filterx.option1=value1
     :
   l4l.appender.appenderName.filterx.optionN=valueN

   Configuring loggers

   The syntax for configuring the root logger is:

   l4l.rootLogger=[level],appenderName[,appenderName]...
   ...
   l4l.logger.loggerName=[inherited|level],appenderName[,appenderName]...
   ...
   l4l.additive.loggerName=true|false
   ...
   l4l.renderer.nameOfRenderedClass=nameOfRenderingClass

   As in,

   l4l.renderer.TFruit=TFruitRenderer
   ...
   l4l.loggerFactory=nameOfLoggerFactoryClass
   ...
   l4l.appender.Mem1.layout.pattern=%7r [%-8t] %p %c %x - %m%n
   # Add a string filter
   l4l.appender.Mem1.filter1=TLogStringFilter
   # That matches on 'x'
   l4l.appender.Mem1.filter1.match=x
   # And accepts all messages containing it
   l4l.appender.Mem1.filter1.acceptOnMatch=false
   # Add a second string filter
   l4l.appender.Mem1.filter2=TLogStringFilter
   # That matches on 'y'
   l4l.appender.Mem1.filter2.match=y
   # And discards all messages containing it
   # Note: messages with 'x' and 'y' will be logged as filter 1 is checked first
   l4l.appender.Mem1.filter2.acceptOnMatch=false

   l4l.appender.Mem2=TMemoAppender
   l4l.appender.Mem2.memo=memMyappMore
   l4l.appender.Mem2.layout=TLogSimpleLayout

   l4l.appender.Mem3=TMemoAppender
   l4l.appender.Mem3.memo=memMyappOther
   l4l.appender.Mem3.layout=TLogHTMLLayout

   l4l.appender.Mem4=TMemoAppender
   l4l.appender.Mem4.memo=memAlt
   l4l.appender.Mem4.layout=TLogPatternLayout
   l4l.appender.Mem4.layout.pattern=>%m<%n

   # Create a file appender
   l4l.appender.Fil1=TLogFileAppender
   l4l.appender.Fil1.filename=C:\Temp\l4l.log
   l4l.appender.Fil1.errorHandler=TLogOnlyOnceErrorHandler
   l4l.appender.Fil1.layout=TLogPatternLayout
   l4l.appender.Fil1.layout.pattern=%r [%t] %p %c %x - %m%n

   # Nominate renderers - when objects of type TEdit are presented,
   # use TComponentRenderer to display them
   l4l.renderer.TEdit=TComponentRenderer
