...
const
  ...
  { Defining this value as true makes log4l print internal debug
...
{ Loggers ---------------------------------------------------------------------}
  ...
  { This is the central class in the log4l package. One of the distinctive
    features of log4l is hierarchical loggers and their evaluation. }
...
{ Layouts ---------------------------------------------------------------------}
  ...
  { Abstract base for layouts.
    ...
    log4l.appender.<name>.layout.dateFormat=yyyy/mm/dd hh:nn:ss.zzz
  }
  ...
  { This layout outputs events in a HTML table.
    ...
    log4l.appender.<name>.layout.title=Logging Messages
  }
  ...
  { Layout based on specified pattern.
    ...
    log4l.appender.<name>.layout.pattern=%r [%t] %p %c %x - %m%n
  }
...
{ Filters ---------------------------------------------------------------------}
  ...
  { Abstract base class for filters - handles basic option setting.
    ...
    log4l.appender.<name>.filter1=TLogCustomFilter
    # Accept or reject the log event when deciding, Boolean, optional, default true
    log4l.appender.<name>.filter1.acceptOnMatch=true
  }
  ...
  { Filter by the message's level.
    ...
    # Class identification
    log4l.appender.<name>.filter1=TLogLevelMatchFilter
    # Logging level to match on, Level, mandatory
    log4l.appender.<name>.filter1.match=warn
  }
  ...
  { Filter by the message's level being within a range.
    ...
    # Class identification
    log4l.appender.<name>.filter1=TLogLevelRangeFilter
    # Minimum logging level to match on, Level, mandatory
    log4l.appender.<name>.filter1.minimum=warn
    # Maximum logging level to match on, Level, mandatory
    log4l.appender.<name>.filter1.maximum=error
  }
  ...
  { Filter by text within the message.
    ...
    # Class identification
    log4l.appender.<name>.filter1=TLogStringFilter
    # Text to match on anywhere in message, string, mandatory
    log4l.appender.<name>.filter1.match=xyz
    # Whether match is case-sensitive, Boolean, optional, defaults to false
    log4l.appender.<name>.filter1.ignoreCase=true
  }
...
{ Appenders -------------------------------------------------------------------}
  ...
  { Send log messages to a file.
    ...
    # Class identification
    log4l.appender.<name>=TLogFileAppender
    # Name of the file to write to, string, mandatory
    log4l.appender.<name>.fileName=C:\Logs\App.log
    # Whether to append to file, Boolean, optional, defaults to true
    log4l.appender.<name>.append=false
  }
  ...
  { Send log messages to a file which uses logfile rotation
    ...
    # Class identification
    log4l.appender.<name>=TLogRollingFileAppender
    # Name of the file to write to, string, mandatory
    log4l.appender.<name>.fileName=C:\Logs\App.log
    # Whether to append to file, Boolean, optional, defaults to true
    log4l.appender.<name>.append=false
    # Max. file size accepts suffix "KB", "MB" and "GB", optional, default 10MB
    log4l.appender.<name>.maxFileSize=10MB
    # Max number of backup files, optional, default is 1
    log4l.appender.<name>.maxBackupIndex=3
  }
...
{ Configurators ---------------------------------------------------------------}
  ...
  { Extends BasicConfigurator to provide configuration from an external file.
    ...
    It is sometimes useful to see how log4l is reading configuration files.
    You can enable log4l internal logging by defining the log4l.debug variable. }
...
resourcestring
  ...
  PleaseInitMsg           = 'Please initialise the log4l system properly';
...
begin
  Result := IsEnabledFor(log4l.Debug);
end;
...
procedure TLogLog.DoLog(const LogLevel: TLogLevel; const Message: string;
  const Err: Exception);
begin
  if (LogLevel <> log4l.Debug) or InternalDebugging then
...
{ Read configuration from properties. The existing configuration is not
  ...
  log4l.appender.appenderName=nameOfAppenderClass

  # Set appender specific options.
  log4l.appender.appenderName.option1=value1
    :
  log4l.appender.appenderName.optionN=valueN

  For each named appender you can configure its ErrorHandler.
  The syntax for configuring an appender's error handler is:

  log4l.appender.appenderName.errorHandler=nameOfErrorHandlerClass

  For each named appender you can configure its Layout.
  The syntax for configuring an appender's layout and its options is:

  log4l.appender.appenderName.layout=nameOfLayoutClass
  log4l.appender.appenderName.layout.option1=value1
    :
  log4l.appender.appenderName.layout.optionN=valueN
  ...
  log4l.appender.appenderName.filterx=nameOfFilterClass
  log4l.appender.appenderName.filterx.option1=value1
    :
  log4l.appender.appenderName.filterx.optionN=valueN

  Configuring loggers

  The syntax for configuring the root logger is:

  log4l.rootLogger=[level],appenderName[,appenderName]...
  ...
  log4l.logger.loggerName=[inherited|level],appenderName[,appenderName]...
  ...
  log4l.additive.loggerName=true|false
  ...
  log4l.renderer.nameOfRenderedClass=nameOfRenderingClass

  As in,

  log4l.renderer.TFruit=TFruitRenderer
  ...
  log4l.loggerFactory=nameOfLoggerFactoryClass
  ...
  log4l.appender.Mem1.layout.pattern=%7r [%-8t] %p %c %x - %m%n
  # Add a string filter
  log4l.appender.Mem1.filter1=TLogStringFilter
  # That matches on 'x'
  log4l.appender.Mem1.filter1.match=x
  # And accepts all messages containing it
  log4l.appender.Mem1.filter1.acceptOnMatch=false
  # Add a second string filter
  log4l.appender.Mem1.filter2=TLogStringFilter
  # That matches on 'y'
  log4l.appender.Mem1.filter2.match=y
  # And discards all messages containing it
  # Note: messages with 'x' and 'y' will be logged as filter 1 is checked first
  log4l.appender.Mem1.filter2.acceptOnMatch=false

  log4l.appender.Mem2=TMemoAppender
  log4l.appender.Mem2.memo=memMyappMore
  log4l.appender.Mem2.layout=TLogSimpleLayout

  log4l.appender.Mem3=TMemoAppender
  log4l.appender.Mem3.memo=memMyappOther
  log4l.appender.Mem3.layout=TLogHTMLLayout

  log4l.appender.Mem4=TMemoAppender
  log4l.appender.Mem4.memo=memAlt
  log4l.appender.Mem4.layout=TLogPatternLayout
  log4l.appender.Mem4.layout.pattern=>%m<%n

  # Create a file appender
  log4l.appender.Fil1=TLogFileAppender
  log4l.appender.Fil1.filename=C:\Temp\log4l.log
  log4l.appender.Fil1.errorHandler=TLogOnlyOnceErrorHandler
  log4l.appender.Fil1.layout=TLogPatternLayout
  log4l.appender.Fil1.layout.pattern=%r [%t] %p %c %x - %m%n

  # Nominate renderers - when objects of type TEdit are presented,
  # use TComponentRenderer to display them
  log4l.renderer.TEdit=TComponentRenderer
